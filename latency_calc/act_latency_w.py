...
def main(arg_log, arg_slice, arg_num_buckets, arg_every_nth, arg_extra):
	...
	# Make sure commit-to-device was configured:
	file_id.seek(0, 0)
	line = file_id.readline()
	while line and not line.startswith("commit-to-device"):
		line = file_id.readline()
	if not line:
		print "commit-to-device not configured"
		sys.exit(-1)
	elif not line.split(" ")[1].startswith("y"):
		print "commit-to-device not enabled"
		sys.exit(-1)
